            ...
            <div id="preloader"
                ...
                <div style="text-align:center;background:white;padding:40px;border-radius:15px;box-shadow:0 10px 40px rgba(0,0,0,0.2);max-width:450px;width:90%;border:1px solid #e0e0e0;">
                    ...
                    <div style="margin-top:25px;">
                        <h5 style="color:#333;font-weight:600;font-size:1.25rem;">Uploading GL File...</h5>
                        <p style="color:#666;margin-bottom:20px;">Please wait while the file is being uploaded...</p>
                        ...
                        <div class="modal-body">
                            <div class="card mb-3">
                                <div class="card-header bg-light">
                                    <h6 class="mb-0"><i class="fa fa-info-circle me-2"></i>Document Details</h6>
                                </div>
                ...
                <div class="modal-dialog modal-dialog-centered modal-xl" role="document">
                    <div class="modal-content">
                        ...
                        <div class="modal-footer">
                            ...
                            <button type="button" class="btn btn-secondary" id="cancelViewBtn">
                                            ...
                                            <tr id="row-{{ $index }}">
                                                ...
                                                <td class="text-center" style="white-space:nowrap;">
                                                    <button type="button"
                                                        ...
                                                        data-bank1="{{ $row['PLC_LOCACODE']        ?? 'N/A' }}"
                                                        data-remarks="{{ $row['PVT_VCHTTYPE']      ?? 'N/A' }}"
                                                        data-amount="{{ number_format((float)($row['LSB_SDTLCRAMTFC'] ?? 0), 2) }}">
...
.btn-print-custom:hover {
    ...
    },
    {
        ...
        $(document).on('click', '.btn-upload', function () {
            ...
            currentDocNum = $(this).data('doc');
            $('#modalDocNum').text($(this).data('doc')      || '-');
            $('#modalRepName').text($(this).data('repname') || '-');
            $('#modalBank1').text($(this).data('bank1')     || '-');
            $('#modalAmount').text($(this).data('amount')   || '-');
            $('#modalRemarks').text($(this).data('remarks') || '-');
            resetFile();
            $('#uploadModal').modal('show');
        });
        $('#cancelUploadBtn, #uploadModal .close').on('click', function () { $('#uploadModal').modal('hide'); });
        $('#uploadModal').on('click', function (e) { if ($(e.target).is('#uploadModal')) $(this).modal('hide'); });

        //  View Document Modal 
        $(document).on('click', '.btn-view-doc', function () {
            var docNum = $(this).data('doc');
            if (!docNum) { showErrorToast('No document number found.'); return; }

            $('#viewDocTitle').text('— ' + docNum);
            $('#viewDocLoading').show();
            ...
            $.ajax({
                ...
                success: function (res) {
                    ...
                    if (!res.success || !res.file_name) {
                        $('#viewDocFallbackName').text(res.message || 'No document found for: ' + docNum);
        ...
        $('#cancelViewBtn, #viewDocModal .close').on('click', function () {
            $('#viewDocModal').modal('hide');
            $('#viewDocFrame').empty();
        });
        $('#viewDocModal').on('click', function (e) {
            if ($(e.target).is('#viewDocModal')) { $(this).modal('hide'); $('#viewDocFrame').empty(); }
        });

        // Drop zone fro uplaoding file  
        var $zone = $('#dropZone');
        ...
        $('#fileInput').on('change', function () { if (this.files.length) setFile(this.files[0]); });
    ...
    $('#fileInput')[0].files = dt2.files;
        ...
        $('#btnUpload').on('click', function () {
            ...
            fd.append('doc_num',  currentDocNum);
            ...
            $.ajax({
                url: '{{ url("claim-payment") }}/' + encodeURIComponent(currentDocNum),
                method: 'POST',
                data: fd,
                processData: false,
                contentType: false,
                success: function (res) {
                    ...
                    if (res.success) {
                        var rowNode = document.getElementById('row-' + currentId);
                        if (rowNode && glDataTable) { glDataTable.row(rowNode).remove().draw(false); }
                        showSuccessToast('GL file uploaded for Doc: ' + res.doc_num);
                    } else {
